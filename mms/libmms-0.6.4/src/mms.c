...
static int fallback_io_tcp_connect(void *data, const char *host, int port)
{
  ...
  for (r = res; r != NULL; r = r->ai_next) {
    ...
    ret = connect(s, r->ai_addr, r->ai_addrlen);
        ...
        do { 
           ...
           if (ret < 0 && errno != EINTR) { 
              ...
              if (getsockopt(s, SOL_SOCKET, SO_ERROR, (void*)(&valopt), &lon) < 0) { 
                 lprintf("Error in getsockopt() %d - %s\n", errno, strerror(errno)); 
                 goto error; 
              } 
              // Check the value returned... 
              if (valopt) { 
                 lprintf("Error in delayed connection() %d - %s\n", valopt, strerror(valopt) );
                 goto error; 
              } 
              break; 
           } 
           else { 
              lprintf("Timeout in select() - Cancelling!\n"); 
              goto error; 
           } 
        } while (1); 
      } 
    ...
    if (setsockopt (s, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv,
                sizeof(tv)) < 0)
        lprintf("setsockopt SO_RCVTIMEO failed\n");

    if (setsockopt (s, SOL_SOCKET, SO_SNDTIMEO, (char *)&tv,
                sizeof(tv)) < 0)
        lprintf("setsockopt SO_SNDTIMEO, failed\n");
#endif
     freeaddrinfo(res);
     return s;
